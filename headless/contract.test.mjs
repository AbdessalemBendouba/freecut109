...
const samples = {
  ...
  addItem: { op: 'addItem', item: { type: 'text', id: 'i', trackId: 'v', from: 0, durationInFrames: 1 } },
  ...
  addKeyframe: { op: 'addKeyframe', itemId: 'i', property: 'opacity', frame: 0, value: 1, easing: 'linear' },
  removeKeyframes: { op: 'removeKeyframes', itemId: 'i', property: 'effect:gpu-blur:e1:radius' },
  addEffect: { op: 'addEffect', itemId: 'i', gpuEffectType: 'gpu-gaussian-blur', params: { radius: 2 } },
...
test('every published edit discriminator has a valid strict schema', () => {
  ...
  for (const op of EDIT_OPERATION_NAMES) assert.equal(editOpSchema.safeParse(samples[op]).success, true, op)
  ...
  assert.equal(editOpSchema.safeParse({ op: 'addEffect', itemId: 'i', gpuEffectType: 'gpu-invented' }).success, false)
  assert.equal(editOpSchema.safeParse({ op: 'updateItem', id: 'i', updates: {} }).success, false)
  assert.equal(editOpSchema.safeParse({ op: 'setTransform', id: 'i', transform: {} }).success, false)
})
...
test('edit request requires exactly one project source and nonempty valid ops', () => {
  ...
  assert.equal(editRequestSchema.safeParse({ project: 'p', projectObject: {}, ops: [samples.addText] }).success, false)
...
test('render request enforces finite bounds, ranges, enums, and canonical HTTP fields', () => {
  assert.equal(renderRequestSchema.safeParse({ project: 'p', fps: 1, resolution: '16x16' }).success, true)
  assert.equal(renderRequestSchema.safeParse({ project: 'p', fps: 240, resolution: '16384x16384' }).success, true)
  for (const invalid of [
    { project: 'p', fps: 0 }, { project: 'p', fps: Number.NaN },
    { project: 'p', resolution: '15x1080' }, { project: 'p', quality: 'best' },
    { project: 'p', inSec: 2, outSec: 1 }, { project: 'p', duration: 0 },
    { project: 'p', in: 1 }, { project: 'p', outSec: 2, duration: 1 },
    { project: 'p', container: 'mp3' }, { project: 'p', audioOnly: true, container: 'webm' },
  ]) assert.equal(renderRequestSchema.safeParse(invalid).success, false, JSON.stringify(invalid))
  assert.deepEqual(normalizeRenderInput({ project: 'p', in: '1', 'out-sec': '2', 'audio-only': true }), {
    project: 'p', inSec: 1, outSec: 2, audioOnly: true,
  })
})

test('validation errors and capabilities are machine-readable and bounded', () => {
  assert.throws(() => validate(editRequestSchema, { project: 'p', ops: [] }), (error) => {
...
test('CLI rejects unknown options and normalizes aliases', () => {
  ...
  assert.deepEqual(parseArgs(['--in', '2'], { allowed, aliases: { in: 'inSec' } }), { _: [], inSec: '2' })
...
test('project listings expose distinct actionable directory ids', () => {
  ...
  try {
    for (const dir of ['one', 'two']) {
      ...
      fs.writeFileSync(path.join(projectDir, 'project.json'), JSON.stringify({ id: 'duplicate', name: dir, updatedAt: 1 }))
    }
    const listed = listProjects(root)
    assert.deepEqual(listed.map((entry) => entry.id).sort(), ['one', 'two'])
    assert.deepEqual(listed.map((entry) => entry.projectId), ['duplicate', 'duplicate'])
