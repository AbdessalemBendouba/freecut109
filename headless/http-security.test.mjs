...
import {
  assertSinglePathComponent, readJsonBody, resolveContained, sendHttpError, setHttpTimeouts,
...
function fixture() {
  ...
  fs.writeFileSync(path.join(workspace, 'projects', 'safe-project', 'project.json'), JSON.stringify({ id: 'safe' }))
...
function rawRequest(base, requestPath, { method = 'GET', headers } = {}) {
  ...
  return new Promise((resolve, reject) => {
    const req = http.request({ host: url.hostname, port: url.port, path: requestPath, method, headers }, (res) => {
      const chunks = []
...
test('ids reject both separator styles and workspace lookups remain contained', () => {
  ...
  try {
    for (const id of ['../escape', '..\\escape', '/absolute', 'C:\\absolute', '.', '..']) {
      assert.throws(() => assertSinglePathComponent(id), (error) => error.code === 'INVALID_ID')
      assert.throws(() => loadProjectById(workspace, id), (error) => error.code === 'INVALID_ID')
      assert.throws(() => resolveMediaFile(workspace, id), (error) => error.code === 'INVALID_ID')
    }
...
test('canonical containment rejects traversal and sibling-prefix paths', () => {
  ...
  try {
    assert.throws(() => resolveContained(dist, '../dist-secret/secret.txt'), (error) => error.code === 'PATH_OUTSIDE_ROOT')
    ...
    try {
      fs.symlinkSync(path.join(root, 'dist-secret'), link, process.platform === 'win32' ? 'junction' : 'dir')
      assert.throws(() => resolveContained(dist, 'outside-link/secret.txt'), (error) => error.code === 'PATH_OUTSIDE_ROOT')
    } catch (error) {
...
test('harness rejects encoded traversal, malformed encoding, and non-read methods', async () => {
  ...
  try {
    ...
    assert.equal((await rawRequest(server.base, '/assets/%2e%2e/%2e%2e/dist-secret/secret.txt')).status, 403)
...
test('media server implements valid single ranges and 416 rejections', async () => {
  ...
  try {
    const range = await rawRequest(server.base, '/media/safe-media', { headers: { Range: 'bytes=2-5' } })
    assert.equal(range.status, 206)
    assert.equal(range.body.toString(), '2345')
    assert.equal(range.headers['content-range'], 'bytes 2-5/10')
    const suffix = await rawRequest(server.base, '/media/safe-media', { headers: { Range: 'bytes=-3' } })
    assert.equal(suffix.body.toString(), '789')
    for (const invalid of ['bytes=20-30', 'bytes=5-2', 'bytes=0-1,3-4', 'items=0-2', 'bytes=-0']) {
      const response = await rawRequest(server.base, '/media/safe-media', { headers: { Range: invalid } })
    ...
    assert.equal((await rawRequest(server.base, '/media/safe-media', { method: 'POST' })).status, 405)
  } finally {
    await server.close()
    fs.rmSync(root, { recursive: true, force: true })
  }
...
test('body reader counts bytes and rejects oversized chunked bodies', async () => {
  ...
  try {
    ...
    const response = await new Promise((resolve, reject) => {
      const req = http.request({ host: url.hostname, port: url.port, method: 'POST', headers: { 'Transfer-Encoding': 'chunked' } }, (res) => {
        res.resume()
...
test('aborted file streams are cleaned up and do not poison the server', async () => {
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      const req = http.get({ host: url.hostname, port: url.port, path: '/media/safe-media' }, (res) => {
