...
function AxisInput({
  ...
  onCommit: (value: number, options: { allowCreate: boolean }) => void
}) {
  ...
  const commit = (allowCreate: boolean) => {
    ...
    if (Number.isFinite(parsed)) {
      if (parsed !== value) onCommit(parsed, { allowCreate })
    }
...
export function CompoundPropertyInputs({ config }: { config: CompoundPropertyInputConfig }) {
  return (
    <div
      className="flex w-[116px] shrink-0 items-center overflow-hidden rounded-sm border border-border/70"
